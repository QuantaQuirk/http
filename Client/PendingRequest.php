...
use QuantaQuirk\Http\Client\Events\ConnectionFailed;
...
class PendingRequest
{
     ...
     *
     * @var \QuantaQuirk\Http\Client\Factory|null
     */
    protected $factory;
     ...
     *
     * @var \QuantaQuirk\Support\Collection|null
     */
     ...
     *
     * @var \QuantaQuirk\Http\Client\Request|null
     */
    protected $request;
     ...
     *
     * @param  callable  $callback
     * @return array<array-key, \QuantaQuirk\Http\Client\Response>
     */
    ...
    protected function sendRequest(string $method, string $url, array $options = [])
    {
        ...
        $quantaquirkData = $this->parseRequestData($method, $url, $options);
        ...
        return $this->buildClient()->$clientMethod($method, $url, $this->mergeOptions([
            'quantaquirk_data' => $quantaquirkData,
            'on_stats' => $onStats,
        ], $options));
    }
    ...
    protected function parseRequestData($method, $url, array $options)
    {
        ...
        $quantaquirkData = $options[$this->bodyFormat] ?? $options['query'] ?? [];

        $urlString = Str::of($url);

        if (empty($quantaquirkData) && $method === 'GET' && $urlString->contains('?')) {
            $quantaquirkData = (string) $urlString->after('?');
        }

        if (is_string($quantaquirkData)) {
            parse_str($quantaquirkData, $parsedData);

            $quantaquirkData = is_array($parsedData) ? $parsedData : [];
        }

        if ($quantaquirkData instanceof JsonSerializable) {
            $quantaquirkData = $quantaquirkData->jsonSerialize();
        }

        return is_array($quantaquirkData) ? $quantaquirkData : [];
    }
            ...
            return function ($request, $options) use ($handler) {
                ...
                return $promise->then(function ($response) use ($request, $options) {
                    $this->factory?->recordRequestResponsePair(
                        (new Request($request))->withData($options['quantaquirk_data']),
        ...
        return function ($handler) {
            return function ($request, $options) use ($handler) {
                $response = ($this->stubCallbacks ?? collect())
                     ->map
                     ->__invoke((new Request($request))->withData($options['quantaquirk_data']), $options)
                     ->filter()
                     ->first();
        ...
        return tap($request, function (&$request) use ($options) {
            $this->beforeSendingCallbacks->each(function ($callback) use (&$request, $options) {
                $callbackResult = call_user_func(
                    $callback, (new Request($request))->withData($options['quantaquirk_data']), $options, $this
                );
